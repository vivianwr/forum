...
class OneFileLoginApplication
{
     ...
     private function changePassword()
    {
        ...
        $user_password_hash = password_hash($user_password, PASSWORD_DEFAULT);
        $sql = 'SELECT user_name, user_email, user_password_hash FROM users WHERE user_name = :user_name OR user_email = :user_email LIMIT 1';
        $query = $this->db_connection->prepare($sql);
        $query->bindValue(':user_name', $user_name);
        $query->bindValue(':user_email', $user_email);
        $query->execute();
        ...
        if ($result_row) {
            if (password_verify($_POST['user_password'], $result_row->user_password_hash)) {
                $sql = 'UPDATE users SET user_password_hash = :user_password_hash WHERE user_name = :user_name OR user_email = :user_email';
                $query = $this->db_connection->prepare($sql);
                $query->bindValue(':user_name', $user_name);
                $query->bindValue(':user_password_hash', $user_password_hash);
                $query->bindValue(':user_email', $user_email);
    ...
    private function showPageLoginForm()
    {
        ...
        echo '</form>';
        $change_role .= "<a href='quickstart1.php?'>Register with your Facebook' role</a>";
        echo $change_role;
